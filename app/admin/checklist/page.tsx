...
export default function ChecklistPage() {
  ...
  // Syncs newly-created checklist items with child's logged session counts
  ...
  async function ensureChecklist(childId: string): Promise<ChecklistWithItems> {
    ...
    if (!newCl) {
      const { data: fallback } = await supabase
        .from('weekly_checklists').select('*, checklist_items(*, chore_templates(*))').eq('child_id', childId).eq('week_start', weekStart).single()
      if (fallback) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {children.map(child => {
        ...
        // Only show extra chores assigned to this child
